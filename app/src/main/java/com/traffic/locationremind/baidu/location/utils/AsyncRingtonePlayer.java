...
public final class AsyncRingtonePlayer {
     ...
     */
    public static Uri getArriveRingtoneUri(Context context) {
        return Utils.getResourceUri(context, R.raw.arrive);
    }

    /**
     * @return Uri of the ringtone to play when the chosen ringtone fails to play
     */
    public static Uri getChangeRingtoneUri(Context context) {
        return Utils.getResourceUri(context, R.raw.chang);
    }
    ...
    private class MediaPlayerPlaybackDelegate implements PlaybackDelegate {
        ...
        public boolean play(final Context context, Uri ringtoneUri, long crescendoDuration) {
            ...
            Log.i(TAG,"Play ringtone via android.media.MediaPlayer. ringtoneUri = "+ringtoneUri);
            ...
            Uri alarmNoise = inTelephoneCall ? getArriveRingtoneUri(context) : ringtoneUri;
            ...
            try {
                ...
                Log.e(TAG,"Using the fallback ringtone, could not play " + alarmNoise, t);
                // The alarmNoise may be on the sd card which could be busy right now.
                // Use the fallback ringtone.
                try {
                    // Must reset the media player to clear the error state.
                    mMediaPlayer.reset();
                    mMediaPlayer.setDataSource(context, getArriveRingtoneUri(context));
                    return startPlayback(inTelephoneCall);
                } catch (Throwable t2) {
    ...
    private class RingtonePlaybackDelegate implements PlaybackDelegate {
        ...
        public boolean play(Context context, Uri ringtoneUri, long crescendoDuration) {
            ...
            final boolean inTelephoneCall = isInTelephoneCall(context);
            if (inTelephoneCall) {
                ringtoneUri = getArriveRingtoneUri(context);
            }
            ...
            try {
                ...
                Log.e(TAG,"Using the fallback ringtone, could not play " + ringtoneUri+t);
                // Recover from any/all playback errors by attempting to play the fallback tone.
                mRingtone = RingtoneManager.getRingtone(context, getArriveRingtoneUri(context));
                try {
                    return startPlayback(inTelephoneCall);
                } catch (Throwable t2) {
                    // At this point we just don't play anything.
        ...
        private boolean startPlayback(boolean inTelephoneCall) {
            ...
            /*else if (mCrescendoDuration > 0) {
                //setRingtoneVolume(AudioManager.);
        ...
        public boolean adjustVolume(Context context) {
            ...
            //final float volume = computeVolume(System.currentTimeMillis(), mCrescendoStopTime, mCrescendoDuration);
